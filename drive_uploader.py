# drive_uploader.py - VERSIÓN COMPLETA CON SUBIDA Y DESCARGA DE HISTÓRICOS
import os
import pickle
import time
import io
...
def authenticate_drive():
    """
    Autentica con Google Drive usando credenciales OAuth 2.0.
    Retorna el servicio de Drive o None si falla.
    """
    ...
    # 1. Intentar cargar credenciales desde token.json
    ...
    # 2. Si no hay token válido, iniciar flujo OAuth
...
def upload_file_to_drive(service, filename, filepath, mime_type, retry_count=0):
    ...
    try:
        ...
        try:
            ...
            print(f"   🔓 Archivo configurado como público", flush=True)
        except Exception as e:
...
def download_history_from_drive(service, history_folder="historial_cooperativo"):
    """
    Descarga los archivos históricos (cooperativo_*.html y audios) desde Drive.
    Retorna True si al menos se pudo descargar algo.
    """
    print(f"\n📥 Descargando históricos desde Google Drive a '{history_folder}'...", flush=True)
...
def upload_history_folder(service, history_folder="historial_cooperativo", max_files=10):
    """
    Sube los archivos más recientes de la carpeta histórica a Drive.
    Retorna una lista con los nombres de los archivos subidos.
    """
...
if __name__ == '__main__':
    ...
    if service:
        ...
        download_history_from_drive(service)
        print("🧪 Probando subida de históricos...")
        upload_history_folder(service, "historial_cooperativo", max_files=5)
    else:
