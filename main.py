# main.py - VERSION CON IMPORTACIÓN DE Dict CORREGIDA
import os
import sys
import json
import shutil
...
from typing import Dict, Any, List, Optional  # 🔥 Importaciones completas
...
try:
    from drive_uploader import upload_file_to_drive, authenticate_drive, upload_history_folder
...
def update_topic_index(summaries: Dict, topic_index) -> int:
    ...
    if tip_title:
        if topic_index.add_tip(tip_title, tip_text):
            added_tips += 1
            print(f"  💡 Nuevo TIP agregado: '{tip_title}'", flush=True)
        else:
...
def run_cooperative_agent():
    ...
    try:
        ...
        # ============================================
        print("\n☁️ PASO 7: Subiendo a Google Drive...", flush=True)
        ...
        # ============================================
        print(f"\n🗑️ PASO 8: Limpiando historico (manteniendo {MAX_HISTORY_DAYS} dias)...", flush=True)
        cleanup_old_history()
