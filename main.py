# main.py - VERSIÓN COMPLETA Y OPTIMIZADA CON SINCRONIZACIÓN INICIAL CON DRIVE
import os
import json
import shutil
import sys
...
def sync_from_drive() -> bool:
    ...
    pero solo si la variable PARENT_FOLDER_ID está configurada.
    """
    ...
    try:
        ...
        # Descargar todos los archivos históricos (HTML y MP3)
        ...
        return True  # Continuamos aunque falle la descarga
...
def upload_to_drive() -> bool:
    ...
    try:
        ...
        regional_audio = f"resumen_cooperativo_{datetime.now().strftime('%Y-%m-%d')}.mp3"
        if os.path.exists(regional_audio):
            print(f"📤 Subiendo: {regional_audio}")
            upload_file_to_drive(service, regional_audio, regional_audio, 'audio/mpeg')
        elif os.path.exists("resumen_cooperativo.mp3"):
            ...
            print("⚠️ No se encontró audio regional")
        
        # 4. Subir audios por país
        country_codes = ['CO', 'PA', 'CR', 'DO']
        for code in country_codes:
            audio_file = f"audio_{code}_{datetime.now().strftime('%Y-%m-%d')}.mp3"
...
def run_cooperative_agent() -> bool:
    ...
    try:
        ...
        # ============================================
        # PASO 5: GENERAR AUDIO DEL RESUMEN REGIONAL
        # ============================================
        ...
        if regional_summary and len(regional_summary) > 50:
            ...
            if audio_file and os.path.exists(audio_file):
                ...
                print(f"✅ Audio regional generado: {audio_file}")
            else:
                print("⚠️ No se pudo generar el audio regional")
        else:
            print("ℹ️ No hay resumen regional suficiente para generar audio")
