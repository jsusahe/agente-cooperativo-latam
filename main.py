# main.py - VERSION ACTUALIZADA CON IA-COOP-LAB Y TOPIC INDEX
import os
import json
import shutil
import sys
...
def sync_from_drive():
    ...
    try:
        ...
        # Tambien cargar el indice de temas
...
def upload_to_drive():
    ...
    try:
        ...
        if os.path.exists("topic_index.json.tmp"):
...
def update_topic_index(summaries: Dict):
    ...
    try:
        ...
        service = authenticate_drive()
        if not service:
            print("No se pudo autenticar para actualizar indice")
            return False
        ...
        for code, summary in summaries.items():
            ...
            for topic in key_topics:
                if index.add_topic(topic, country):
                    added += 1
        ...
        if added > 0:
            ...
            print("No se agregaron nuevos temas")
        
        return True
        
    except Exception as e:
        print("Error al actualizar indice: {}".format(e))
        return False
...
def run_cooperative_agent():
    ...
    try:
        ...
        print("\nPASO 9: Limpiando historico (manteniendo {} dias)...".format(MAX_HISTORY_DAYS))
        cleanup_old_history()
        
        print("\nPASO 10: Subiendo a Drive...")
        upload_to_drive()
