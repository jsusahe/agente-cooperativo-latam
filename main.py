# main.py - VERSION FUSIONADA CON DESCARGA DE HISTÓRICOS DESDE DRIVE
import os
import sys
import json
import shutil
...
def upload_to_drive(html_file, audio_files=None):
    """
    ...
    try:
        ...
        uploaded = upload_history_folder(HISTORY_FOLDER, max_files=10)
        if uploaded:
...
def run_cooperative_agent():
    ...
    # 🔥 Inicializar índice de temas (desde Drive o local)
    print("\n📚 Cargando índice de temas...", flush=True)
    try:
        service = authenticate_drive() if os.environ.get('PARENT_FOLDER_ID') else None
    ...
    try:
        ...
        # ============================================
        # PASO 5: GENERAR HTML Y AUDIOS
        # ============================================
        print("\n📄 PASO 5: Generando boletin HTML y audios...", flush=True)
        ...
        # ============================================
        # PASO 6: COPIAR A LA RAIZ
        # ============================================
        print("\n🌐 PASO 6: Copiando archivos a la raiz...", flush=True)
        ...
        # ============================================
        # 🔥 PASO 7: DESCARGAR HISTÓRICOS DESDE DRIVE
        # ============================================
        print("\n📥 PASO 7: Descargando históricos desde Drive...", flush=True)
        if os.environ.get('PARENT_FOLDER_ID') and service:
            try:
                # 🔥 Descargar los últimos 20 archivos históricos para asegurar tener al menos 10
                download_success = download_history_from_drive(HISTORY_FOLDER)
                if download_success:
                    print("✅ Históricos descargados correctamente", flush=True)
                else:
                    print("⚠️ No se pudieron descargar históricos (puede que no haya archivos en Drive)", flush=True)
            except Exception as e:
                print(f"⚠️ Error al descargar históricos: {e}", flush=True)
        else:
            print("ℹ️ No se configuró Drive o no hay autenticación. Usando históricos locales existentes.", flush=True)
        
        # ============================================
        # PASO 8: SUBIR A DRIVE
        # ============================================
        print("\n☁️ PASO 8: Subiendo a Google Drive...", flush=True)
        upload_success = upload_to_drive(html_file, audio_files)
