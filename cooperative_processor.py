...
class CooperativeProcessor:
    ...
    def process_country_news(self, country_data: Dict) -> Dict:
        ...
        for article in articles:
            ...
            # Asegurar que published no sea None para evitar errores de comparación
    ...
    def select_news_for_summary(self, articles: List[Dict], grouped: Dict) -> List[Dict]:
        ...
        # ✅ FUNCIÓN DE ORDENAMIENTO SEGURA - FIX DEL ERROR
        def safe_sort_key(item):
            """Función de ordenamiento que maneja None de forma segura"""
            published = item.get('published')
            if published is None:
                return datetime(1970, 1, 1)  # Fecha muy antigua como fallback
            return published
        ...
        for category in priority_order:
            if category in grouped and grouped[category]:
                # Tomar la más reciente de cada categoría con ordenamiento seguro
                sorted_news = sorted(
                    grouped[category], 
                    key=safe_sort_key, 
                    reverse=True
        ...
        if remaining_slots > 0:
            ...
            # Ordenar por fecha con ordenamiento seguro
