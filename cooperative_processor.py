...
import time as time_module  # Para convertir struct_time
...
class CooperativeProcessor:
    ...
    def _normalize_date(self, date_value) -> datetime:
        ...
        Maneja: None, struct_time, datetime, string
        """
        ...
        # Si es struct_time (de feedparser)
    ...
    def process_country_news(self, country_data: Dict) -> Dict:
        ...
        # Limpiar y clasificar cada artículo
        for article in articles:
            ...
            # 🔧 NORMALIZAR FECHA - Convertir struct_time a datetime
            if article.get('published'):
                article['published'] = self._normalize_date(article['published'])
            else:
                article['published'] = datetime.now()
        
        # Agrupar por categoría de fuente
    ...
    def select_news_for_summary(self, articles: List[Dict], grouped: Dict) -> List[Dict]:
        ...
        # ✅ FUNCIÓN DE ORDENAMIENTO SEGURA - Maneja todos los tipos
        def safe_sort_key(item):
            """Función de ordenamiento que maneja cualquier tipo de fecha"""
            published = item.get('published')
            if published is None:
                return datetime(1970, 1, 1)
            # Si es struct_time, convertirlo
        ...
        # Intentar obtener al menos una de cada categoría prioritaria
        for category in priority_order:
            if category in grouped and grouped[category]:
                # Tomar la más reciente de cada categoría
                sorted_news = sorted(
                    grouped[category], 
                    key=safe_sort_key, 
                    reverse=True
        ...
        # Llenar el resto con noticias adicionales
