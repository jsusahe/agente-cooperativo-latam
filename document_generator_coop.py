# document_generator_coop.py - VERSIÓN CORREGIDA CON EXPORTACIÓN CORRECTA
import json
import os
import shutil
...
def find_audio_file(base_name: str, extensions: List[str] = ['.mp3']) -> Optional[str]:
    ...
    locations = [
        '.',  # Raíz del proyecto
        'historial_cooperativo',  # Carpeta de histórico
    ]
    
    for location in locations:
        for ext in extensions:
            # Probar con el nombre base
            filename = f"{base_name}{ext}"
            filepath = os.path.join(location, filename)
            if os.path.exists(filepath):
                return filepath
            
            # Probar sin fecha (para archivos genéricos)
            if '_' in base_name and len(base_name.split('_')) > 2:
                # Intentar con el nombre sin fecha
...
def get_audio_url(audio_path: str) -> str:
    ...
    # Si está en historial_cooperativo/, la URL debe incluir la carpeta
    if audio_path.startswith('historial_cooperativo/'):
        return audio_path
    
    # Si está en la raíz, solo el nombre del archivo
...
def generate_cooperative_html(summaries: Dict, output_filename="index.html") -> str:
    ...
    # Si no se encuentra con fecha, buscar el genérico
    if not regional_audio_found:
        regional_audio_found = find_audio_file("resumen_cooperativo")
    
    # Determinar URL del audio regional
    regional_audio_url = get_audio_url(regional_audio_found) if regional_audio_found else None
    
    print(f"🔍 Audio regional encontrado: {regional_audio_found}")
    print(f"   URL para HTML: {regional_audio_url}")
...
<body>
    ...
    if regional.get('has_news'):
        ...
        # Reproductor de audio regional (usando la URL encontrada)
...
if __name__ == '__main__':
    ...
    test_data = {
        'CO': {
            'country': 'Colombia',
            'has_news': True,
            'summary': 'El sector cooperativo colombiano presenta avances significativos en regulación y supervisión.',
            'news_count': 8,
            'key_topics': ['Regulación', 'Crecimiento'],
            'news_items': []
        },
        'REGIONAL': {
            ...
            'trends': ['Inclusión financiera', 'Regulación armonizada']
        }
    }
