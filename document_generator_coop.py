# document_generator_coop.py - CON IMPORTACIÓN DE Optional
import json
import os
import shutil
from datetime import datetime
from typing import Dict, List, Optional  # ⬅️ AGREGAR Optional AQUÍ
...
<body>
    ...
    if regional.get('has_news'):
        ...
        if regional_audio_exists:
            audio_src = regional_audio_file if os.path.exists(regional_audio_file) else "resumen_cooperativo.mp3"
            html_content += f"""
                <div class="audio-player">
...
def generate_audio_for_country(country_code: str, country_name: str, summary_text: str) -> Optional[str]:
    ...
    if not summary_text or len(summary_text) < 50:
        print(f"⚠️ Texto muy corto para generar audio de {country_name}")
        return None
...
def generate_audio_for_region(regional_summary: str) -> Optional[str]:
    ...
    if not regional_summary or len(regional_summary) < 50:
        print("⚠️ Texto regional muy corto para generar audio")
        return None
...
def generate_cooperative_document(summaries: Dict) -> str:
    ...
    for code in country_codes:
        ...
        if summary.get('has_news') and summary.get('summary'):
            ...
            print(f"  🔊 Generando audio para {country}...")
            audio_file = generate_audio_for_country(code, country, summary.get('summary', ''))
            if audio_file:
                audio_files.append(audio_file)
                print(f"    ✅ Audio generado: {audio_file}")
            else:
                print(f"    ⚠️ No se pudo generar audio para {country}")
    
    # Generar audio regional
    print("\n🎧 Generando audio regional...")
    regional = summaries.get('REGIONAL', {})
    if regional.get('has_news') and regional.get('summary'):
        ...
        if regional_audio:
            ...
            print(f"  ✅ Audio regional generado: {regional_audio}")
        else:
            print("  ⚠️ No se pudo generar audio regional")
