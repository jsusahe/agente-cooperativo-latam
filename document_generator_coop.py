...
def generate_cooperative_html(summaries: Dict, output_filename="index.html") -> str:
    """Genera el HTML del boletín cooperativo"""
...
<body>
        ...
        if has_news:
            ...
            if news_items:
                ...
                for item in news_items[:5]:
                    ...
                    badge_class = f"badge-{category}" if category in ['supervision', 'national_association', 'regional_association', 'employee_fund', 'solidarist', 'media', 'cooperative', 'video', 'guarantee'] else 'badge-other'
                    category_display = {
                    ...
                    html_content += f"""
                        <div class="news-item">
                            ...
                            {f'<div class="news-summary">{summary_text}</div>' if summary_text else ''}
            ...
            html_content += f"""
                <div class="no-news">
                    ...
                    <p style="font-size: 0.85rem;">Intenta ampliar el período de búsqueda o verifica las fuentes.</p>
                </div>
            """
    ...
    total_news = sum(s.get('news_count', 0) for s in summaries.values() if s.get('has_news'))
    ...
    html_content += f"""
        ...
        <div class="footer">
            ...
            <p style="font-size: 0.7rem; color: #a0aec0; margin-top: 10px;">
                Código abierto en GitHub · Reporta problemas o sugiere fuentes
            </p>
        </div>
    </div>
...
def generate_cooperative_document(summaries: Dict) -> str:
    """Genera el documento completo del boletín cooperativo"""
    date_str = datetime.now().strftime("%Y-%m-%d")
    filename = f"cooperativo_{date_str}.html"
    
    # También guardar como index.html para GitHub Pages
...
if __name__ == '__main__':
    ...
    test_data = {
        ...
        'REGIONAL': {
            ...
            'has_news': True,
            'summary': 'El cooperativismo latinoamericano muestra un crecimiento sostenido, con innovaciones en inclusión financiera y fortalecimiento de los marcos regulatorios. La región avanza hacia una mayor integración de los sistemas cooperativos.',
